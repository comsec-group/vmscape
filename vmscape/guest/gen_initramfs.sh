...
function usage() {
    ...
    echo "   -o, --out <FILE>       File path to store the initramfs.cpio.gz (default: $out)"
...
if [ ! -d $INITRAMFS_DIR ]; then
    ...
    mount_p9=""
    if [ "$share" = true ]; then
        mount_p9="mount -t 9p         host0       /mnt  -o trans=virtio,version=9p2000.L"
    fi
...
insmod /mnt/pi.ko
insmod /mnt/rap.ko
echo 2049 > /sys/devices/system/node/node0/hugepages/hugepages-2048kB/nr_hugepages
$cwd
$init
EOF
    chmod +x init
