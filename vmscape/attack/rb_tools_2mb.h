...
// #define RB_SZ ROUND_1GB_UP((RB_SLOTS * RB_STRIDE) + 0x1000UL)
...
static __always_inline void rb_init(void) {
    ...
    if (mmap((void *) RB_PTR, RB_SZ, PROT_READ | PROT_WRITE,
             RB_MMAP_FLAGS | MAP_FIXED_NOREPLACE | MAP_HUGETLB | MFD_HUGE_2MB, -1,
             0) == MAP_FAILED) {
        err(1, "rb");
    }
     ...
     * reloading. */
    for (uint64_t i = RB_PTR; i < RB_PTR + RB_SZ; i += 4096) {
        *(volatile uint64_t *) (i) = i;
    }
}
